...
内核构建目录依然是上次构建的linux-4.9。

LED是Orange Pi One板子上自带的两个LED，一红一绿，它们的引脚都接在GPIO控制器上。

红LED的引脚是GPIO的A组的15号引脚。

绿LED的引脚是GPIO的L组的10号引脚。

Orange Pi One用的SoC是Allwinner H3，这部分内容位于技术手册的p316到p350。

A到G组是一个GPIO控制器控制的，内容从p316到p344。

L组是单独的一个GPIO控制器控制的，内容从p345到p350。
...
cfg是配置寄存器（每个寄存器由4位配置），dat是数据（0是低电平，1是高电平），pull是上拉电阻和下拉电阻设置（这是为了让电平稳定，主要是输入才用到，输出是用不到的）。

参考我之前在 [HOS项目中的LED驱动代码](https://github.com/HuangCheng72/HOS/blob/main/Program_OrangePiOne/devices/led/led.c) 。

结合先前开发的ring_buf驱动的模板，就可以写出对应的驱动代码如下：
...
编译，把编译出来的myled.ko模块复制到TF卡，上Orange Pi One启动，用insmod命令挂载驱动。

```bash
/ # cd home
/home # insmod myled.ko
[   12.072346] myled: loading out-of-tree module taints kernel.
[   12.078528] Registered correctly with major number 250
[   12.083793] Device class registered correctly
[   12.088405] Device class created correctly
[   12.092564] Device initialized
/home # cat[   17.658276] random: fast init done
-/bin/sh: c: not found
/home # cat /dev/myled
[   27.763956] myled: Device has been opened
Green LED status: ON, Red LED status: OFF[   27.768078] myled: Device successfully closed

/home # echo "1" > /dev/myled
[   50.387812] myled: Device has been opened
[   50.391981] myled: Red LED turned ON
[   50.395568] myled: Green LED turned OFF
[   50.399457] myled: Device successfully closed
/home # echo "1" > /dev/myled
[   60.543472] myled: Device has been opened
[   60.547601] myled: Red LED turned OFF
[   60.551290] myled: Green LED turned ON
[   60.555053] myled: Device successfully closed

```

经过实际测试，完全成功。这是我第一个真正操控硬件的Linux驱动。